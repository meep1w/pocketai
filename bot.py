...
from db import (
    init_db, get_session, get_or_create_user, User, AsyncSession,
    ...
    async with get_session() as session:
        ...
        if p is not None:
            msg = await bot.send_photo(
                chat_id=db_user.telegram_id, photo=FSInputFile(p),
...
async def evaluate_and_route(bot: Bot, user: User) -> None:
    """
    Показывает актуальный следующий экран (с учётом включённых проверок).
    Автообновляет подписку, регистрацию, депозит. «Доступ открыт» — один раз.
    """
    async with get_session() as session:
        ...
        # Platinum флаг (на всякий случай — тут, но основное приходит из постбэков)
...
async def cb_instructions(c: CallbackQuery, bot: Bot):
    async with get_session() as session:
        user = await get_or_create_user(session, c.from_user.id)
        # Теперь кнопка регистрации — callback (btn_register)
        await send_screen(
...
# --- Новое: кнопка «Я подписался» в шаге подписки ---
...
async def on_check_subscription(c: CallbackQuery):
    async with get_session() as session:
        ...
        try:
            is_sub = await check_subscription(c.bot, user.telegram_id)
        except Exception:
            is_sub = False
...
# --- Новое: кнопка регистрации из инструкции (callback) ---
...
async def on_btn_register(c: CallbackQuery, bot: Bot):
    async with get_session() as session:
        ...
        # показываем шаг регистрации (с URL-кнопкой)
